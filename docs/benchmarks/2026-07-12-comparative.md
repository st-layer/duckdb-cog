...
| 1,000점 포인트 샘플링 | **7.4 ms** (RS_Values‡) / 20.7 ms (RS_Value) | 31.3 ms *(정합 실패 — 참고치)*§ | 18.3 ms | n/a† |
...
‡ **에라텀 (2026-07-12 정정, 이슈 #27)**: 초판은 이 칸을 73.6 ms 로 싣고 "산개
포인트에서 배치가 스칼라보다 느리다(산개 휴리스틱 필요)"고 결론냈으나, 그 수치는
duckdb-python 의 **리스트 쿼리-파라미터 변환 비용**(1k×2 리스트 ~107 ms, 래스터
무관)이 지배한 측정 오류였다. 스칼라 루프와 같은 pts 테이블에서 `list(x)/list(y)`
집계로 조립해 재측정하면 완전 산개 1,000점에서도 배치가 **7.4 ms 로 3× 빠르다**
(스칼라 20.7 ms 는 원판과 동일 — 배치 측정만 오염). 산개 휴리스틱 백로그는 소멸.
...
¶ GDAL vsicurl 은 프로세스 전역 캐시를 가져 두 번째 접근부터 0.03s. 측정 당시 우리는
연결/청크 단위 dedupe 만 있었으나, *(2026-07-12 갱신)* #26 으로 프로세스 전역 리더
캐시가 착지 — 동일 URL 웜 재접근 0.3–0.8ms (TTL 60s, `COG_REMOTE_CACHE_TTL_S`).
**진짜 콜드는 동급(≈2s)**.
...
- 포인트 샘플링을 행별 스칼라(RS_Value 20.7ms)로 쓰면 PostGIS(18.3ms)가 근소 우위 —
  배치(RS_Values 7.4ms‡)로 쓰면 우리가 앞선다. "파일 도착 → 첫 답"의 총비용은
  어느 형태든 우리가 압도 (0 vs 1.4s+α).
- ~~반복 원격 접근은 GDAL 의 전역 캐시가 유리~~ *(2026-07-12 해소: #26 프로세스
  전역 리더 캐시 착지 — 동일 URL 웜 재접근 0.3–0.8ms, ¶ 참조)*.
