...
| `RS_BandStats(path[, band])` | STRUCT(min,max,mean,stddev) | 태그 부재·범위 밖 밴드·NULL 인자 → NULL | Sedona 에 없는 확장 (§6.7): COG GDAL_METADATA 의 STATISTICS_* — decode 없는 카탈로그 통계. fallback 은 RS_ZonalStats |
| `RS_BandAsArray(path, band[, zone])` | DOUBLE[] | 범위 밖 밴드·NULL 인자 → NULL (빈 배열과 구분); nodata → NULL **원소** | bbox 없으면 전체 level 0 밴드(georef 불요 — **통째 실체화되므로 대형 COG 는 bbox 윈도 권장**), 있으면 픽셀 중심 포함 윈도 (zonal 과 동일 규약 — Sedona 에 없는 windowed 확장). row-major (좌상→우하). **zone 오버로드(#53)**: bbox `DOUBLE[4]` 또는 WKT `VARCHAR`(POLYGON/MULTIPOLYGON) — WKT 는 폴리곤 **envelope** 창을 돌려주되 중심이 폴리곤 밖인 픽셀은 NULL 원소로 가린다 (nodata 와 구분하지 않음). 픽셀 중심 포함 규약을 `RS_ZonalStats` 폴리곤과 공유하므로 같은 zone 이면 두 함수가 보는 픽셀 집합이 정확히 같다 (엔진 테스트가 count·sum 일치로 고정) |
| `RS_ZonalStats(path, zone, band, stat)` | DOUBLE | NULL 인자·bbox 원소 NULL → NULL; 유효 픽셀 없음 → count 0, 나머지 NULL | **부분 이탈**: zone 오버로드 = bbox `DOUBLE[4]` 또는 WKT `VARCHAR`(POLYGON/MULTIPOLYGON, 구멍 포함 — #48). Sedona 는 geometry 타입 인자, 우리는 GEOS 비링크(N4) 유지를 위해 WKT 문자열 + 순수 Rust even-odd PIP (DuckDB spatial GEOMETRY blob 수용은 후속). 픽셀 **중심** 포함 규약, nodata 제외 — bbox 는 닫힌 구간(공유 경계 위 중심은 이웃 zone 양쪽에 포함), 폴리곤 경계 **위**의 중심은 비계약(half-open; 오라클 픽스처는 중심 격자 비정렬 꼭짓점으로 회피). zone 은 raster native CRS 전제(N2 — 재투영 없음). 범위 밖 밴드도 빈 집계와 동일 (count 0 / 나머지 NULL — RS_Value 의 NULL 과 비대칭, 의도). EMPTY·extent 밖 폴리곤 → 빈 집계 (bbox 역전은 에러인 것과 대비). stat ∈ count/sum/mean/min/max (대소문자 무관); 모르는 stat·bbox 형식 오류·WKT 파싱 실패·비폴리곤 타입 → 에러. **배치 오버로드(#60, Sedona 에 없는 확장)**: zone 에 `VARCHAR[]`(WKT 목록) → `DOUBLE[]` 입력 순서 반환 — zone 별 의미론은 스칼라와 동일, NULL 원소는 NULL 자리로 보존, 리스트 인자 NULL 은 행 NULL. 엔진이 전체 zone 의 타일 union 을 1회 fetch+decode 하므로 "작은 zone 다수" 워크로드의 호출당 고정비를 상각한다 |
