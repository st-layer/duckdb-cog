...
| `RS_NormalizedDifference(path, x, y, b1, b2)` | DOUBLE | 결측(extent 밖·nodata·범위 밖 밴드)·**합 0**·NULL 인자 → NULL | **의도적 이탈**: Sedona 는 raster 반환 — 우리는 reader(N3)라 포인트 값 연산. (v2−v1)/(v2+v1) |
...
`RS_BandAsArray`, `RS_ZonalStats` — RFC §6.8 표 참조. 래스터
